...
export async function GET() {
    try {
      ...
      SELECT s.spacecraft_id, s.name, s.model, m.fuel_level, s.health_status, s.mission_id, m.name as mission_name
      FROM spacecraft s
...
export async function POST(req: Request) {
    try {
        const { name, model, mission_id } = await req.json();
        ...
        const result = await pool.query(
            "INSERT INTO spacecraft (name, model, health_status, mission_id) VALUES ($1, $2, 'Operational', $3) RETURNING *",
            [name, model, mission_id || null]
        );
